...
async function get(extra = {}) {
  ...
  const base = (data.result ?? data.aliexpress_ds_product_get_response?.result)
    ?.ae_item_base_info_dto;
  return base?.sales_count;
}

console.log("default", await get());
console.log("with ext", await get({ pdp_ext_f: JSON.stringify({ order: "14", fromPage: "search" }) }));
