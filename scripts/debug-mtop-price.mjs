...
for (const k of ["PRICE", "PRICE_EXTEND", "PRICE_BANNER", "QUANTITY_PC", "BOTTOM_BAR_PC", "SKU"]) {
  fs.writeFileSync(`scripts/mtop-${k}.json`, JSON.stringify(d[k] ?? null, null, 2));
  console.log("wrote", k);
}

console.log("\nselectedSkuId:", d.PRICE?.selectedSkuId);
