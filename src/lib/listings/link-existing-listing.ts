...
export function buildDefaultLinkVariants(
  product: Awaited<ReturnType<typeof fetchListingProductSource>>,
): LinkExistingVariantInput[] {
  ...
  return variants.map((variant) => ({
    ...
    listedQuantity: Math.max(1, variant.stock ?? product.stock ?? 1),
    sku: "",
    offerId: "",
  }));
}
...
async function buildDraftForLinkedListing(
  ...
  let draft: ListingDraft = {
    ...
    listing: {
      ...
      suggestedPrice: variants[0]?.listedPrice ?? suggestListedPrice(product.price),
